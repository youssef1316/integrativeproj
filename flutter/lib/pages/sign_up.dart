...
class _SignUpScreenState extends State<SignUpScreen> {
  ...
  //Email verification function (sends verification email and checks if link is used)
  Future<void> _showEmailVerificationDialog(User user) async {
    ...
    void startCooldown(StateSetter setState) {
              ...
              content: Column(
                ...
                children: [
                  const Text(
                    ...
                    'Please open it and confirm your address.'
                  ),
                  const SizedBox(height: 12),
                  if (isChecking) const LinearProgressIndicator(),
                  if (!isChecking && !canResend && _cooldown > 0)
                    Text('You can resend in $_cooldown s'),
                ],
              ),
              actions: [
                TextButton(
                  onPressed: canResend
                      ? () async {
                          try {
                            ...
                            if (!mounted) return;
                            _showSnackBar('Verification email re-sent.', isError: false);
                            setState(() { /* trigger cooldown */ });
                            startCooldown(setState);
                          } catch (e) {
                            if (!mounted) return;
                            _showSnackBar('Failed to resend: $e');
                          }
                        }
                      : null,
                  ...
                  onPressed: () async {
                    ...
                    try {
                      ...
                      if (refreshed != null && refreshed.emailVerified) {
                        ...
                        if (!mounted) return;
                        _showSnackBar("Still not verified. Please click the link in your email.");
                      }
                    } finally {
